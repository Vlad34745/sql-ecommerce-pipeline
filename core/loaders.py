        ...
        conn.executemany(
            """INSERT INTO users(source_user_id, name, email, registration_date)
               ...
               ON CONFLICT(source_user_id) DO UPDATE SET
                 name=excluded.name, email=excluded.email, registration_date=excluded.registration_date""",
            [(u["source_user_id"], u["name"], u["email"], u["reg_date"].isoformat()) for u in users],
        )
        conn.executemany(
            """INSERT INTO orders(source_order_id, source_user_id, source_product_id, user_id, product_id, order_date, quantity)
               SELECT ?, ?, ?, u.user_id, p.product_id, ?, ?
               FROM users u CROSS JOIN products p
               WHERE u.source_user_id=? AND p.source_product_id=?
               ON CONFLICT(source_order_id) DO UPDATE SET
                 order_date=excluded.order_date, quantity=excluded.quantity,
                 user_id=excluded.user_id, product_id=excluded.product_id""",
            [(o["source_order_id"], o["source_user_id"], o["source_product_id"], o["order_date"].isoformat(sep=" "),
              o["quantity"], o["source_user_id"], o["source_product_id"]) for o in orders],
        )
        ...
        with conn.cursor() as cur:
            ...
            cur.executemany(
                """INSERT INTO orders(source_order_id, source_user_id, source_product_id, user_id, product_id, order_date, quantity)
                   ...
                   FROM users u CROSS JOIN products p
                   WHERE u.source_user_id=%s AND p.source_product_id=%s
                   ON CONFLICT(source_order_id) DO UPDATE SET
                     order_date=EXCLUDED.order_date, quantity=EXCLUDED.quantity,
                     user_id=EXCLUDED.user_id, product_id=EXCLUDED.product_id""",
                [(o["source_order_id"], o["source_user_id"], o["source_product_id"], o["order_date"],
                  o["quantity"], o["source_user_id"], o["source_product_id"]) for o in orders],
            )
